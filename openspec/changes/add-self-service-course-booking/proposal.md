...
- 指派教师或授权 Admin 确认预约时，Hono 在一个 PocketBase SDK Batch 中完成时间占位、正式课堂创建、学员课时预留、教师分配、预约状态、审计与 Outbox。
- 新增数据库唯一约束支持的离散日历占位，防止学员预约与 Admin 排课在并发条件下重复占用教师或学员时间。
- Admin 增加预约管理工作区，用于查看待处理队列、代教师确认或拒绝预约、配置预约规则与教师课程资格、处理取消和冲突；已确认预约继续进入现有课堂与核销流程。
...
- `ops-admin-system`: 增加待处理预约决策、预约运营、教师课程资格、预约规则和冲突处理的角色与 capability 约束工作流。
...
- `ikanyue.admin`: 新增 `/appointments` 工作区、待处理预约确认/拒绝、预约规则和教师可用时间管理，并让现有课堂生命周期统一维护时间占位。
