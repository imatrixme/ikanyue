...
### get_simulator_console — 读取 console 日志

对小程序 console 缓冲区执行 grep 过滤并返回命中行。

```bash
wechatide -c <clientName> -t get_simulator_console --project <project> --command "grep -i error"
```
...
### get_simulator_network — 读取 network 请求

对小程序 network 缓冲区执行 grep 过滤并返回命中行。

```bash
wechatide -c <clientName> -t get_simulator_network --project <project> --command "grep -n ."
```
...
| 用户意图 | 选择 |
|---------|------|
| "给我看报错日志" | `get_simulator_console`（command: `"grep -i error"`） |
| "接口为什么没回来" | `get_simulator_network`（command: `"grep -n ."`） |
