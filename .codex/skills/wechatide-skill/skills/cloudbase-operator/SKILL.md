...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `appid` | string | 否 | 小程序 AppID；与 `project` 都填时优先使用 `appid` |
| `project` | string | 否 | 项目绝对路径；未传 `appid` 时从 `project.config.json` 读取 |
...
按云函数目录路径完整部署单个云函数到指定云环境，属于写操作。一次只允许部署一个云函数目录，目录名即函数名称。

```bash
wechatide -c <clientName> -t cloud_fn_deploy --appid <appid> --env <envId> --path <cloudFunctionDir> --remote-npm-install
```
...
| `env` | string | 是 | 云环境 ID |
| `path` | string | 是 | 云函数目录绝对路径，目录名即函数名称 |
| `remote-npm-install` | boolean | 否 | 是否远程安装 npm |

---

### cloud_fn_inc_deploy — 增量部署云函数

按云函数目录路径增量部署变更文件或目录到指定云环境，属于写操作。固定使用 `path` + `appid` 链路，函数目录名即函数名称。
...
### cloud_query_storage — 读取云存储

只读查询云存储文件列表、文件信息、临时下载链接或文本内容。

```bash
wechatide -c <clientName> -t cloud_query_storage --appid <appid> --env <envId> --action list --cloud-path <cloudPath>
```
...
### cloud_manage_storage — 修改云存储

上传、下载或删除云存储文件。`upload` / `delete` 是写操作，必须经过用户确认；`download` 不触发确认。

```bash
wechatide -c <clientName> -t cloud_manage_storage --appid <appid> --env <envId> --action upload --cloud-path <cloudPath> --local-path <localPath>
```
...
| "把这个函数部署上去" | `cloud_fn_deploy`（`appid` + `path`） |
...
| "列出云存储文件" | `cloud_query_storage`（action=list） |
| "获取云存储临时下载链接" | `cloud_query_storage`（action=url） |
| "读取云存储文本内容" | `cloud_query_storage`（action=read） |
| "上传 / 删除云存储文件" | `cloud_manage_storage` |
| "下载云存储文件到本地" | `cloud_manage_storage`（action=download，无需确认） |

## 选择提醒

- 如果环境名还不明确，先做 `cloud_env_list`
- 如果只是想确认状态，优先函数只读查询，不要直接部署
- **部署云函数固定使用 `appid + path`**，明确传入云函数目录的本地绝对路径
- **增量部署云函数固定使用 `appid + path + file`**；`file` 必须相对云函数目录
...
- `cloud_query_storage` 的 `action=url` 和 `cloud_manage_storage` 的 upload 返回临时签名链接，不可当作永久公开 URL
...
- `cloud_query_storage`：列出文件、获取文件信息、临时下载链接、读取文本内容。
- `cloud_manage_storage`：上传、下载、删除云存储文件；upload/delete 需用户确认，delete 还需 `force=true`。

## 使用边界

- 当存在多个候选环境时，绝不自动选择其中一个
- 部署云函数固定使用 `appid + path` 组合
- 云数据库写操作必须经过用户确认
