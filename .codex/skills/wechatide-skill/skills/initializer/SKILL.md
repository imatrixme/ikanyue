...
### check_wechatide_status — 检查 微信开发者工具 状态
...
**重要：当工具调用返回当前的版本较低时，务必提醒用户，并从返回的内容中的 `skillPath` 中更新本 skill**

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `version` | string | 否 | 当前调用侧 agent 加载的 `wechatide-skill/skill.yaml` 文件中的 `version` 字段值；不传则跳过版本对比提醒 |
...
4. 如果 `project.config.json` 不存在或 `appid` 无效：
   ...
   - 让用户选择 AppID
   - 创建或补全 `project.config.json`（参考 `wechatide-tools/references/open-project-window-guide.md`）
...
获取当前登录用户可管理的全部 AppID 列表。可通过 `type` 按小程序/小游戏过滤。

```bash
wechatide -c <clientName> -t get_user_appids
wechatide -c <clientName> -t get_user_appids --type miniprogram
wechatide -c <clientName> -t get_user_appids --type minigame
```

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `type` | string | 否 | 类型：`all`(默认)、`miniprogram`、`minigame` |
...
项目设置请直接读写 `project.config.json` / `project.private.config.json`，见 `skills/project-config/SKILL.md`。`project_setting_get` / `project_setting_update` 已废弃，调用只会返回废弃提示。
...
| "看看现在登没登录" | `check_wechatide_status` |
| "换一个登录用户" | `login` |
| "关闭项目窗口" | `close_project_window` |
| "退出 微信开发者工具" | `quit` |
| "帮我看看我能用的 AppID" | `get_user_appids` |
| "当前页面是什么" | `automation_runtime_info`（action: `currentPage`） |
| "看看项目设置" / "改一下项目设置" | 使用 `skills/project-config/SKILL.md`（读写 `project.config.json` / `project.private.config.json`，不要调 MCP） |
| "构建 npm" | `buildnpm` |
