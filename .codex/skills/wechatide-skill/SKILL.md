...
```bash
wechatide -c <clientName> -t check_wechatide_status --skill-version <skillVersionFromSkillYaml>
```

其中 `<skillVersionFromSkillYaml>` 不是固定字符串；调用前应读取当前调用侧 agent 加载的 `wechatide-skill/skill.yaml`，取该文件顶层 `version` 字段的值填入。

`check_wechatide_status` 通过 skill-cli 调用时，如果 微信开发者工具 MCP 服务未启动或当前 client 尚未授权，CLI 会使用同一个 `<clientName>` 自动执行一次等价的 `wechatide auth -c <clientName>`，授权完成后重试本次状态检查。授权弹窗仍需用户确认。
...
| `skillPath` | 当前微信开发者工具内置的 skill 的路径，当内置 skill 版本与本地 agent 使用版本不一致时，根据此路径更新 skill；然后需要重新执行 `check_wechatide_status`  |

**注意**：`check_wechatide_status` 只需在会话开始时调用一次。确认返回内容中有 `openid` 且无 `warning` 后，后续操作中不要重复调用，直接使用其他工具即可。

### Skill 版本检查

`check_wechatide_status` 会返回当前运行 微信开发者工具 内置的 skill 版本号 `skillVersion`。入参 `version` 应填写当前调用侧 agent 加载的 `wechatide-skill/skill.yaml` 文件中的顶层 `version` 字段值，工具会对比两者；不要硬编码示例版本。版本不一致时返回中会包含 `warning`，应按提示中的工具内置 skill 路径安装更新 skill。未传 `version` 时只检查 微信开发者工具 状态和登录态，不返回版本对比提醒。

### 微信开发者工具 未启动时

通常无需手动执行；`check_wechatide_status` 连接失败时会自动用当前 `<clientName>` 执行一次。需要手动重试时，通过 `skill-cli auth` 启动工具并授权 CLI 连接：
...
```bash
wechatide -c <clientName> -t login
```

等待用户扫码完成后，再次调用 `check_wechatide_status` 确认返回内容中有 `openid`。**未确认登录前不得执行其他工具。**
...
- `-t`：工具名（如 `open_project_window`、`cloud_env_list`）
...
- `object` / `array` 类型字段可直接传 JSON：`--<field> '{"a":1}'`；也可用可选的 `--<field>-file path.json`。二者必填字段只需提供其中一个，不能同时传
...
```bash
wechatide -c CodeBuddy -t open_project_window --project <project>
wechatide -c CodeBuddy -t automation_runtime_info --project <project> --action currentPage
wechatide -c CodeBuddy -t check_wechatide_status --skill-version <skillVersionFromSkillYaml>
...
- **不要猜测或自行编造工具名**，调用前必须先确认工具存在（见 `wechatide-tools/references/tools.yaml`）
- 工具名使用下划线分隔（如 `open_project_window`、`automation_viewport_action`），不要用点号或驼峰
...
- 读写/修改项目配置 `project.config.json`（本地设置、编译开关等，**不调 MCP tool**）：`skills/project-config/SKILL.md`
...
- 需要改项目本地设置 / `project.config.json`（域名校验、热重载、编译压缩等）：`project-config`（直接改文件）
...
5. **如果需要打开项目窗口（`open_project_window`）：必须先读取项目的 `project.config.json`，确认 `appid` 有效后才能调用。** 缺失时走 `open-project-window-guide.md` 补全流程。
6. 选择合适的 scene skill。
...
- 打开项目前置检查：`wechatide-tools/references/open-project-window-guide.md`
- 创建小程序项目流程：`wechatide-tools/references/create-project-guide.md`
- 工具注册表：`wechatide-tools/references/tools.yaml`
