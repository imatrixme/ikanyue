...
调用 `open_project_window` 前，必须确保项目目录中存在有效的 `project.config.json` 且配置了 `appid`。否则 微信开发者工具 无法正确识别和打开项目。
...
- 如果 `appid` 存在且不为空 → 检查通过，可以执行 `open_project_window`
...
读取 <project>/project.config.json
  ├── 文件不存在
  │     └── 确认登录 → get_user_appids → 用户选择 → 创建 project.config.json → open_project_window
  ├── 文件存在但 appid 为空
  │     └── 确认登录 → get_user_appids → 用户选择 → 更新 appid → open_project_window
  └── 文件存在且 appid 有效
        └── 直接 open_project_window
```

## 注意事项

- `get_user_appids` 需要已登录状态（`check_wechatide_status` 返回内容中有 `openid`）
