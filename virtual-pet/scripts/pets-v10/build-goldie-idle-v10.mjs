...
async function prepareDirs() {
  await fs.mkdir(resolveProjectPath(v10Paths.sourceDir), { recursive: true })
  await fs.mkdir(path.dirname(resolveProjectPath(v10Paths.manifestPath)), {
    recursive: true,
  })
  await emptyDir(v10Paths.extractedDir)
  await emptyDir(v10Paths.runtimeDir)
  await emptyDir(v10Paths.reviewDir)
}

async function writeRuntimeFrames(sourceFrames) {
  const runtimePaths = []
  for (let index = 0; index < v10IdleAction.runtimeFrames; index += 1) {
    const frame = await composeRuntimeFrame(
      sourceFrames[index],
      index,
      v10IdleAction.runtimeFrames,
      resolveProjectPath,
    )
...
async function writeReviewStrip(runtimePaths) {
  ...
  await sharp({
    ...
    .toFile(resolveProjectPath(path.join(v10Paths.reviewDir, 'goldie-idle-v10-strip.webp')))
}
...
<head>
  ...
  <title>Goldie V10 Idle Review</title>
  <style>
...
<body>
  <h1>Goldie V10 Idle Review</h1>
  <img src="./goldie-idle-v10-strip.webp" alt="Goldie V10 idle animation strip">
</body>
</html>
`
  await fs.writeFile(resolveProjectPath(path.join(v10Paths.reviewDir, 'index.html')), contents)
}

async function writeManifest(sourceFrames, runtimePaths) {
  const manifest = {
    action: v10IdleAction,
    runtimePaths,
    schemaVersion: 1,
    source: v10Paths.sourcePath,
    sourceFrames,
  }
  await fs.writeFile(resolveProjectPath(v10Paths.manifestPath), `${JSON.stringify(manifest, null, 2)}\n`)
}
...
async function main() {
  await prepareDirs()
  const sourceFrames = await extractSourceFrames(v10Paths.sourcePath, resolveProjectPath)
  const runtimePaths = await writeRuntimeFrames(sourceFrames)
  await writeReviewStrip(runtimePaths)
  await writeReviewIndex()
  await writeManifest(sourceFrames, runtimePaths)
  console.log('Generated Goldie V10 idle frames from direct 32-frame sheet.')
}
